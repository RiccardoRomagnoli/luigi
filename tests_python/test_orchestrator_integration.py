...
class OrchestratorIntegrationTest(unittest.TestCase):
    def test_mocked_end_to_end_loop_applies_changes(self) -> None:
        fixture_src = os.path.join(REPO_ROOT, "tests", "fixtures", "target-project")
        self.assertTrue(os.path.isdir(fixture_src))

        with tempfile.TemporaryDirectory(prefix="orchestrator-it-") as tmp:
            ...
            config = {
                ...
                "claude_code": {
                    "command": ["node", os.path.join(REPO_ROOT, "tests", "mocks", "claude_mock.js")],
                    "model": "opus",
            ...
            self.assertEqual(
                result.returncode,
                0,
                msg=f"Orchestrator failed.\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}\n",
            )

            # Verify the change was applied back into the original target repo path.
